...
class HeatingImport extends ImportHandler
{
    ...
    public function retrieveFromFormatterData(FormatterData $data)
    {
        ...
        while (null !== $row = $data->popRow()) {
            ...
            $energie_tr�ger = utf8_encode($row["energie_tr�ger"]);
            ...
            $w�rmeleistung = $row["w�rmeleistung"];
            $wasserw�rmeleistung = $row["wasserw�rmeleistung"];
            $energieeffizienz_heizung = $row["energieeffizienz_heizung"];
            $effizienz_classe = $row["effizienz_classe"];
            $speicherkapazit�t = $row["speicherkapazit�t"];
            $gewicht = $row["gewicht"];
            $preis = $row["preis"];
            $gc_ek = $row["gc_ek"];
            ...
            if ($productExists == 0) // product_numbers must be unique
            {
				...
				$product_heizung->setPower ( $w�rmeleistung );
				$product_heizung->setEnergyEfficiency ( $energieeffizienz_heizung );
						
						if ($wasserw�rmeleistung == "NULL")
							$product_heizung->setWarmWater ( 0 );
						else
							$product_heizung->setWarmWater ( 1 );
						$product_heizung->setStorageCapacity ( $speicherkapazit�t );
						$product_heizung->setEnergyCarrier($energie_tr�ger);
    ...
    protected function getMandatoryColumns()
    {
        return ["product_id", "sht_id","hersteller_produkt_nummer","bild_name_wurzel","anzahl_von_bilder","marke","produkt_name",
        		"detaillierte_beschreibung","zusammengefaste_beschreibung","produkt_merkmale","category","w�rmeleistung",
        		"wasserw�rmeleistung","energieeffizienz_heizung","energieeffizienz_wasser","effizienz_classe","speicherkapazit�t",
        		"energie_tr�ger","services","dokumente","hersteller_produkt_url","gewicht","preis","gc_ek"];
    }

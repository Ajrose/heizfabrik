...
use HookCalendar\Model\Roles;
use HookCalendar\Model\RolesQuery;
...
class RolesTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'HookCalendar.Model.Map.RolesTableMap';
     ...
     */
    const OM_CLASS = '\\HookCalendar\\Model\\Roles';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'HookCalendar.Model.Roles';
    ...
    protected static $fieldNames = array (
        self::TYPE_PHPNAME       => array('Id', 'Role', 'Status', ),
        self::TYPE_STUDLYPHPNAME => array('id', 'role', 'status', ),
        self::TYPE_COLNAME       => array(RolesTableMap::ID, RolesTableMap::ROLE, RolesTableMap::STATUS, ),
        self::TYPE_RAW_COLNAME   => array('ID', 'ROLE', 'STATUS', ),
        self::TYPE_FIELDNAME     => array('id', 'role', 'status', ),
    ...
    protected static $fieldKeys = array (
        self::TYPE_PHPNAME       => array('Id' => 0, 'Role' => 1, 'Status' => 2, ),
        self::TYPE_STUDLYPHPNAME => array('id' => 0, 'role' => 1, 'status' => 2, ),
        self::TYPE_COLNAME       => array(RolesTableMap::ID => 0, RolesTableMap::ROLE => 1, RolesTableMap::STATUS => 2, ),
        self::TYPE_RAW_COLNAME   => array('ID' => 0, 'ROLE' => 1, 'STATUS' => 2, ),
        self::TYPE_FIELDNAME     => array('id' => 0, 'role' => 1, 'status' => 2, ),
        self::TYPE_NUM           => array(0, 1, 2, )
    );
    ...
    public function initialize()
    {
        ...
        $this->setClassName('\\HookCalendar\\Model\\Roles');
    ...
    {
        if (null === $alias) {
            $criteria->addSelectColumn(RolesTableMap::ID);
            $criteria->addSelectColumn(RolesTableMap::ROLE);
            $criteria->addSelectColumn(RolesTableMap::STATUS);
        } else {
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \HookCalendar\Model\Roles) { // it's a model object
            // create criteria based on pk values
            $criteria = $values->buildPkeyCriteria();
        } else { // it's a primary key, or an array of pks
            $criteria = new Criteria(RolesTableMap::DATABASE_NAME);
            $criteria->add(RolesTableMap::ID, (array) $values, Criteria::IN);
        }

        $query = RolesQuery::create()->mergeWith($criteria);

        if ($values instanceof Criteria) { RolesTableMap::clearInstancePool();
        } elseif (!is_object($values)) { // it's a primary key, or an array of pks
            foreach ((array) $values as $singleval) { RolesTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria->containsKey(RolesTableMap::ID) && $criteria->keyContainsValue(RolesTableMap::ID) ) {

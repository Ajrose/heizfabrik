							...
							<li class="list-group-item pjAsListElement pjAsService">
								<div class="row">
									...
									<div class="col-lg-9 col-md-9 col-sm-9 col-xs-12">
										...
										{ 
											?>
											<a href="#" class="btn btn-default pjAsBtn pjAsBtnPrimary pjAsServiceAppointment" data-id="<?php echo $service['id'];?>" data-slug="<?php echo $slug;?>"><?php __('front_make_appointment');?></a>
											<?php
										}
										?>
								...
								{
									if(isset($_GET['service_id']) && $service['id'] == $_GET['service_id'])
									{
										list($year, $month, $day) = explode("-", $_GET['date']);
										...
										{ 
											...
											<div class="form-horizontal asEmployeeInfo">
												<form class="pjAsAddToCartForm" action="" method="post">
													<div class="form-group">
														...
														<div class="col-lg-9 col-md-9 col-sm-9 col-sx-12">
															...
															{ 
																...
																{
																	?>
																	<select name="employee_id" class="form-control pjAsSelectorEmployee" data-iso="<?php echo $_GET['date'];?>" data-service_id="<?php echo $service['id'];?>" >
																		...
																		foreach($tpl['employee_arr'] as $employee)
																		{
																			if ((int) $tpl['option_arr']['o_seo_url'] === 1)
																			{
																				$slug = sprintf("%s/%s/%s/%s/%s-%u/%s-%u.html", 'Service', $year, $month, $day, pjAppController::friendlyURL($service['name']), $service['id'], pjAppController::friendlyURL($employee['name']), $employee['id']);
																			} else {
																				$slug = NULL;
																			}
																			if(isset($tpl['employee']))
													...
													<input type="hidden" name="service_id" value="<?php echo (int) $tpl['service']['id']; ?>" />

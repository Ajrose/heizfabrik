<?php
if (!defined("PJ_HOST")) define("PJ_HOST", "localhost");
if (!defined("PJ_USER")) define("PJ_USER", "root");
if (!defined("PJ_PASS")) define("PJ_PASS", "admin");
if (!defined("PJ_DB")) define("PJ_DB", "thelia");
if (!defined("PJ_PREFIX")) define("PJ_PREFIX", "");
if (!defined("PJ_INSTALL_FOLDER")) define("PJ_INSTALL_FOLDER", "/schedule/");
if (!defined("PJ_INSTALL_PATH")) define("PJ_INSTALL_PATH", "C:/Development/programs/xampp/htdocs/heizfabrik/web/schedule/");
if (!defined("PJ_INSTALL_URL")) define("PJ_INSTALL_URL", "http://localhost/schedule/");
if (!defined("PJ_SALT")) define("PJ_SALT", "B1HYVS0E");
if (!defined("PJ_INSTALLATION")) define("PJ_INSTALLATION", "");
?>